...
import { demoApi, isDemoMode } from "../worker/demo/api";
import {
  getSettings,
  markCleanupRan,
...
app.route("/api", (demo ? demoApi : api) as typeof api);

app.use("/*", serveStatic({ root: "./dist" }));
app.get("*", serveStatic({ path: "./dist/index.html" }));

async function runCron(bindings: Env): Promise<void> {
  if (isDemoMode(bindings)) return;
...
console.log(`meshflare listening on :${port}${demo ? " (DEMO_MODE)" : ""}`);
